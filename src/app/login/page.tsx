    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            ...
            if (additionalInfo?.isNewUser) {
                // Edge case: User clicked "Google" on the Login screen but they are actually a brand new user
                await setDoc(doc(db, "users", result.user.uid), {
                    name: result.user.displayName || "",
                    email: result.user.email || "",
                    role: "", // No role selected from login page
    ...
    return (
        <main className="min-h-screen flex items-center justify-center bg-[#f9f9f9] p-4">
            <div className="w-full max-w-md bg-white p-8 rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-gray-100">
                ...
                <h1 className="text-[2rem] font-serif text-gray-900 mb-6 text-center">Welcome Back</h1>
