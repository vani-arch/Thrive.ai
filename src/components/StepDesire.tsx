...
interface Props {
    desire: string;
    setDesire: (val: string) => void;
    onNext: () => void;
    onBack: () => void;
}

export default function StepDesire({ desire, setDesire, onNext, onBack }: Props) {
    const maxLength = 500;
        ...
        <div className="w-full max-w-4xl flex flex-col items-center animate-in fade-in duration-500">
            <div className="text-center mb-[3rem] w-full mt-4">
                ...
                <p className="text-[1.2rem] text-gray-500 font-sans tracking-normal mt-2 max-w-2xl mx-auto">
                    If you had a magic wand to instantly solve one bottleneck in your process, what would it do?
                </p>
            </div>
            ...
            <div className="w-full max-w-2xl bg-white p-8 rounded-[12px] shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-gray-100 mb-12 relative">
                <textarea
                    value={desire}
                    onChange={(e) => setDesire(e.target.value.slice(0, maxLength))}
                    placeholder="It would instantly draft the first version of the strategy brief based on raw meeting notes..."
                ...
                <div className="absolute bottom-4 right-6 text-[12px] font-medium text-gray-400">
                    <span className={desire.length >= maxLength ? "text-orange-500" : ""}>
                        {desire.length}
                    </span>
                    {" / "}{maxLength}
                </div>
                ...
                <div className={`absolute bottom-0 left-0 right-0 h-[3px] bg-gradient-to-r from-transparent via-green-500 to-transparent transition-opacity duration-300 ${desire.length > 0 ? 'opacity-100' : 'opacity-0'}`} />
            </div>
          ...
          `}
                >
                    Generate Playbook
                    <svg className={`w-5 h-5 ml-1 transform translate-y-[1px] ${desire.trim().length > 0 ? "text-white" : "text-gray-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
